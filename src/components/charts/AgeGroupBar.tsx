...
import { AlignLeft, BarChart2, Sparkles, Smile, Baby, Rocket, Award } from 'lucide-react';
...
const AGE_CONFIG: Record<string, { label: string; icon: React.ReactNode; bg: string; text: string; chip: string; hex: string }> = {
  '0-1 years': {
    label: 'Infants (0-1 yrs)',
    icon: <Baby className="w-3.5 h-3.5 text-emerald-600" />,
    bg: 'bg-emerald-500',
  ...
  '1-3 years': {
    label: 'Toddlers (1-3 yrs)',
    icon: <Smile className="w-3.5 h-3.5 text-cyan-600" />,
    bg: 'bg-cyan-600',
    text: 'text-cyan-700',
    chip: 'bg-cyan-50 border-cyan-200',
    hex: '#0891B2',
  },
  '3-5 years': {
    label: 'Preschool (3-5 yrs)',
  ...
  '5-8 years': {
    label: 'Early Primary (5-8 yrs)',
    icon: <Rocket className="w-3.5 h-3.5 text-orange-600" />,
    bg: 'bg-orange-500',
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between pb-1 border-b border-purple-50">
        <span className="text-[11px] text-slate-500 font-medium">Multi-tag catalog coverage</span>
        ...
        <div className="space-y-2 max-h-[360px] overflow-y-auto pr-1">
          {data.map((item) => {
            const cfg = AGE_CONFIG[item.label] || {
              label: item.label,
              icon: <Smile className="w-3.5 h-3.5 text-purple-600" />,
              bg: 'bg-purple-600',
              text: 'text-purple-700',
              chip: 'bg-purple-50 border-purple-200',
              hex: '#7C3AED',
              ...
              >
                <div className="flex items-center justify-between mb-1.5 gap-2">
                  <div className="flex items-center gap-1.5 min-w-0">
                    <span className={`p-1 rounded-md ${cfg.chip} border shrink-0`}>
                      {cfg.icon}
                    </span>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <span className={`px-2 py-0.5 rounded-md text-[11px] font-black ${cfg.chip} ${cfg.text} border`}>
                      {item.percentage.toFixed(1)}%
                    </span>
                  </div>
                </div>
